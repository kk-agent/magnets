...
- 2026-03-23 05:22:00 CDT — Built Connected Agents UI: `ConnectedAgentsView.swift` with full agent list (enable/disable toggles, delete confirmation), add-agent sheet (type picker, magnet picker, schedule, custom prompt), per-magnet `MagnetAgentsSheet` accessible from MagnetDetailView toolbar. SettingsView now shows live agent list instead of static placeholder. Build verified + 37/37 tests pass. Committed `cf65c28`, pushed to main. **Phase 4 — Connected Agents UI complete.**
- 2026-03-23 11:58:00 CDT — Added 16 AgentConnection tests: AgentType enums (display names, symbols, prompts, hours, raw values, identifiable, case count), AgentSchedule enums (display names, symbols, identifiable, case count), AgentConnection defaults/custom schedule, effectivePrompt (fallback/custom/whitespace), scheduleDescription (daily/hourly/weekly), Magnet↔Agent relationship, cascade delete. Total tests now 53 (was 37). Committed `4fa3fb0`, pushed to main.
- 2026-03-23 13:58:00 CDT — **Phase 5 competitive analysis complete.** Mapped 10+ competitors across 3 tiers (Locket, Widgetable, Glimpse, NoteIt, Lovestruck, Ekko, etc.). Identified 5 market themes and 5 Magnets differentiators: (1) AI Agents as first-class circle members, (2) On-Device AI Compose, (3) Group-Native architecture, (4) Widget as Living Dashboard, (5) Privacy-First CloudKit design. Execution roadmap with P0/P1/P2 priorities. App Store positioning recommendation with tagline, subtitle, and keywords. Committed `a4333f8`, pushed to main.
